...
export default function Testimonial({
  ...
  return (
    <li className={`flex flex-col gap-6 p-6 smpx-8 sm:py-6 md:flex md:items-start ${className}`}>
      <blockquote className='flex flex-col gap-6 text-left md:grow'>
        <div className='relative text-lg font-medium leading-7 text-gray-600 md:flex-1'>
          ...
          <Paragraph className='relative pl-12 text-left'>{text}</Paragraph>
        </div>
        <footer className='flex items-center gap-4'>
          <div className='flex'>
          <figure className='shrink-0'>
          <img className='size-12 rounded-full border-2 border-white' src={authorAvatar} alt={authorName} data-testid='Testimonial-img' />
            </figure>
